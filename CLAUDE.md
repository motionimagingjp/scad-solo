...
- ホーム画面はシャッフル性を持たない検索画面(「今夜の3軒」)。エンタメ性は
  `/games`タブ(ドリンクルーレット・ソロチン・ソロビンゴ・ソロ乾杯・強制選曲モードの
  5種、いずれも実装済み・仕様確定。大きな追加修正は予定していない)側で担う方針
- AI相談機能(チャットで店舗を相談・提案する機能)は企画段階で廃止が確定した。
  ボトムナビの「AI相談」タブも設けない(下記`BottomNav.tsx`の設計判断を参照)。
  これに伴い、Promptfooによるプロンプト評価計画も対応不要になった(下記「アプリ評価基準」参照)
- 来店ログはスタンプ機能を廃止し、メモ編集+削除に一本化
- DB(シンガポール)を読み書きするAPI/ページは`preferredRegion = "sin1"`を指定
- Neon無料枠のコールドスタート(初回~2秒)はコストとして許容する判断済み

## アプリ評価基準
AI相談機能の廃止に伴い、評価軸を「LLMのプロンプト精度」から以下4点に切り替えた。
今後のレビュー・QAはこの観点で行う。
- **UI/UXの快適さ**: 主要導線(今夜の3軒→チェックイン、ゲーム各種)のタップ数・
  分かりやすさ・レスポンス。「近くのおすすめはこれ」等、挙動が意図通りかユーザーに
  伝わる文言になっているか
- **位置情報・GPSのフォールバック動作**: 手動指定 > GPS > IP推定 > 既定値(東京駅)の
  優先順位が正しく機能するか。GPS拒否時は「システムエラーではなくフォールバックである」
  ことがユーザーに伝わるか(`lib/useBaseLocation.ts`の`gpsDenied`案内バナー)
- **ゲームの動作安定性**: `/games`配下5種(ドリンクルーレット・ソロチン・ソロビンゴ・
  ソロ乾杯・強制選曲モード)が仕様通りに完走し、クラッシュや進行不能状態が無いか
- **データ精度(ハルシネーション不発)**: Spot DBに実在しない店舗・チェーン店の
  誤登録・出典URLの捏造が無いか(既知の落とし穴は本ファイル下部「既知の落とし穴」章参照)
...
- **五反田駅の既存Phase 0データ(13件)を個別検証したところ、錦糸町同様13件
  全滅していた**(とん金・博多劇場・魚金・Bar Amber等、実在する店名/チェーン名に
  架空の住所を組み合わせるパターンと、鶏びあ・なると・昭和酒場・蔵といった
  既知テンプレートの使い回しが混在)。13件を削除し、個別検証済み5件に置き換えた
- 23区・山手線シフト方針のもと優先駅10のうち9駅に着手。投入件数: 有楽町4
  (4回連続0件から初めて投入成功)・大崎7(2バッチ計12件提出→7件)・
  六本木4・神保町4・銀座6・日本橋6・赤坂6・四ツ谷3・日暮里5・原宿4(計49件)。
  「かね子」(飯店/イタリアン系列)が6店舗以上のチェーンと判明したため、
  既存の蒲田駅・町田駅データからも遡って除外した
