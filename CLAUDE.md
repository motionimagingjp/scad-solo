...
- 東京圏10駅とは別に、三島市(静岡県)・大阪市梅田駅を新規エリアとして追加。
  ...
  追加収集は慎重に進める必要がある
- 大井町・目黒・中目黒・三軒茶屋・下北沢の5駅を新規追加。投入件数:
  大井町12・目黒10・中目黒7・三軒茶屋13・下北沢9(計51件)。除外理由の多くは
  ChatGPT提出データで判明した「同一ブランドの複数店舗チェーン」(晩杯屋40+店舗、
  ディプント19+店舗、もつ焼きばん6店舗など)。もつ焼きばんは中目黒店を一度
  KEEPと誤判定したが、下北沢店の検証で6店舗チェーンと判明し中目黒側も遡って
  除外した(同一ブランドは駅ごとに別々に判定せず、横断でチェーン店数を確認する
  必要がある教訓)
- 自由が丘・巣鴨・亀戸・西荻窪・町田の5駅を新規追加。投入件数:
  自由が丘14・巣鴨10・亀戸10・西荻窪13・町田13(計60件)。除外理由は既知
  チェーン(晩杯屋、串カツ田中、ほていちゃん6+店舗、焼売のジョー8+店舗、
  さかなや道場、丸冨水産)、閉店確認(亀戸3件)、店名が実際と異なる(自由が丘1件)
- 大森・武蔵小杉・田町・川口・船橋の5駅を検証。投入件数: 大森3・田町2・
  船橋9・巣鴨に1件追加(Yururi Bar)・西荻窪に1件追加(日本酒・おばんざい梵)。
  **武蔵小杉・川口は0件**(提出候補が全て大手チェーンまたは実在確認不可
  だったため、この2駅は事実上未着手のまま)。この過程で「大衆酒場BEETLE」が
  蒲田・田町・川口にまたがる5店舗以上のチェーンと判明したため、既存の
  蒲田駅データから「酒場BEETLE 蒲田東」を遡って除外した(同一ブランド名が
  複数駅の提出データに分散して出現する場合、駅ごとの店舗数では大手チェーン
  と気づけないことがある教訓。ブランド名で横断検索する習慣が必要)
